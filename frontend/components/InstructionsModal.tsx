...
export const InstructionsModal: React.FC<InstructionsModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 p-4 backdrop-blur-md animate-fade-in" onClick={onClose}>
      <div
        className="panel w-full max-w-sm space-y-5 p-6 text-left animate-scale-in"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-center justify-between border-b border-surface-border pb-3">
          <h3 className="text-sm font-semibold text-slate-900">How to play</h3>
          <button
            type="button"
            onClick={onClose}
            aria-label="Close"
            className="flex h-7 w-7 items-center justify-center rounded-lg border border-surface-border text-muted transition-colors hover:text-slate-900"
          >
        ...
        <div className="space-y-1.5">
          ...
          <p className="text-xs leading-relaxed text-muted">
            Crack the secret <span className="text-slate-800">4-digit code</span>. Every digit is unique (0–9) — no repeats.
          </p>
        </div>
        ...
        <div className="space-y-1.5">
          ...
          <div className="space-y-1 rounded-xl border border-surface-border bg-surface-2 p-3.5 font-mono text-xs">
            ...
            <p><span className="text-muted">Guess&nbsp;</span> &nbsp;<span className="font-bold tracking-[0.3em] text-slate-900">3170</span></p>
